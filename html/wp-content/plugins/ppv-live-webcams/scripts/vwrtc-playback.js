	...
	wsConnection.onmessage = function(evt)
	{
		...
		if (msgStatus == 514) { // repeater stream not ready
			...
			stopPlay();
			 ...
			 console.log( 'wsConnection.onmessage success msgJSON', msgJSON );
			...
			var iceCandidates = msgJSON['iceCandidates'];
			 console.log( 'msgJSON iceCandidates: ', iceCandidates );
				...
				if ('audio'.localeCompare( sdpSection ) == 0) {
					...
					if ( rtpmapID !== null  ) {
						...
						if ( ('vp9'.localeCompare( match ) == 0 ) || ('vp8'.localeCompare( match ) == 0 ) || ('h264'.localeCompare( match ) == 0 ) ||
							...
							if (enhanceData.videoBitrate !== undefined) {
								sdpStrRet += '\r\na=fmtp:' + rtpmapID[1] + ' x-google-min-bitrate=256;x-google-max-bitrate=' + (enhanceData.videoBitrate);
							}
						}
						...
						if ( ('opus'.localeCompare( match ) == 0 ) || ('isac'.localeCompare( match ) == 0 ) || ('g722'.localeCompare( match ) == 0 ) || ('pcmu'.localeCompare( match ) == 0 ) ||
							...
							if (enhanceData.audioBitrate !== undefined) {
								sdpStrRet += '\r\na=fmtp:' + rtpmapID[1] + ' x-google-min-bitrate=32;x-google-max-bitrate=' + (enhanceData.audioBitrate);
							}
						}
					}
				}
