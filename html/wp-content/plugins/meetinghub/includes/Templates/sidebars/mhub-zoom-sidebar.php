	...
	<div class="mhub-sidebar">
						...
						<?php if ( $is_schedule ) { ?>
							...
							<?php } else { ?>
								<span class="mhub-countdown-wrapper">
									<span class="mhub-countdown-value"><?php 2 === $meeting_type ? esc_html_e( 'Meeting is running', 'meetinghub' ) : esc_html_e( 'Webinar is running', 'meetinghub' ); ?></span>
									<span class="mhub-countdown-label"><?php 2 === $meeting_type ? esc_html_e( 'The meeting is started and running', 'meetinghub' ) : esc_html_e( 'The webinar is started and running', 'meetinghub' ); ?></span>
								</span>
							<?php } ?>
					<?php } else { ?>
						<span class="mhub-countdown-wrapper">
							<span class="mhub-countdown-value"><?php 2 === $meeting_type ? esc_html_e( 'Meeting is finished', 'meetinghub' ) : esc_html_e( 'Webinar is finished', 'meetinghub' ); ?></span>
							<span class="mhub-countdown-label"><?php 2 === $meeting_type ? esc_html_e( 'This meeting has been ended by the host.', 'meetinghub' ) : esc_html_e( 'This webinar has been ended by the host.', 'meetinghub' ); ?></span>
						</span>
					<?php } ?>

				<?php } else { ?>
					<span class="mhub-countdown-wrapper">
						<span class="mhub-countdown-value"><?php 2 === $meeting_type ? esc_html_e( 'Meeting does not exist', 'meetinghub' ) : esc_html_e( 'Webinar does not exist', 'meetinghub' ); ?></span>
						<span class="mhub-countdown-label"><?php 2 === $meeting_type ? esc_html_e( 'The meeting is not exists in zoom account.', 'meetinghub' ) : esc_html_e( 'The webinar is not exists in zoom account.', 'meetinghub' ); ?></span>
					</span>
				<?php } ?>
			<?php } else { ?>
				<?php if ( current_user_can( 'manage_options' ) ) { ?>
					<span class="mhub-countdown-wrapper">
						<span class="mhub-countdown-value"><?php 2 === $meeting_type ? esc_html_e( 'Meeting not created', 'meetinghub' ) : esc_html_e( 'Webinar not created', 'meetinghub' ); ?></span>
			...
			<?php if ( ! $meeting_deleted ) { ?>
				<?php if ( current_user_can( 'manage_options' ) || current_user_can( 'tutor_instructor' ) ) { ?>
					<div class="mhub-single-widget mhub-single-widget-host-actions">
						<div class="mhub-widget-inner">
							<?php if ( 'end' !== $meeting_status ) { ?>
							<button  class="mhub-meeting-status" data-meeting-status="end" data-post-id="<?php echo esc_attr( $post->ID ); ?>" data-meeting-id="<?php echo esc_attr( $meeting_id ); ?>"><?php 2 === $meeting_type ? esc_html_e( 'End Meeting ?', 'meetinghub' ) : esc_html_e( 'End Webinar ?', 'meetinghub' ); ?>
							</button>
						<?php } else { ?>
							<button  class="mhub-meeting-status" data-meeting-status="resume" data-post-id="<?php echo esc_attr( $post->ID ); ?>" data-meeting-id="<?php echo esc_attr( $meeting_id ); ?>"><?php 2 === $meeting_type ? esc_html_e( 'Enable Meeting Join ?', 'meetinghub' ) : esc_html_e( 'Enable Webinar Join ?', 'meetinghub' ); ?>
							</button>
						<?php } ?>
							<p><?php 2 === $meeting_type ? esc_html_e( 'You are seeing this because you are the author of this meeting', 'meetinghub' ) : esc_html_e( 'You are seeing this because you are the author of this webinar', 'meetinghub' ); ?></p>
						</div>
					</div>
				<?php } ?>
			<?php } ?>
		...
		<div class="mhub-buy-btn-wpapper">
			...
			if ( ! empty( $product_id ) && 'yes' === $meeting_as_product && ! current_user_can( 'manage_options' ) ) {
				...
				?>
				<a href="<?php echo esc_url( $product_link ); ?>" class="mhub-buy-button" target="__blank"> <?php esc_html_e( 'Buy Now !', 'meetinghub' ); ?></a>
