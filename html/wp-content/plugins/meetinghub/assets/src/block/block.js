...
const GoogleMeetBlockEdit = (props) => {
    const { attributes, setAttributes } = props;
    const { linkOnly, selectedMeeting } = attributes;
    const [loading, setLoading] = useState(true);
    const [meetings, setMeetings] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                formData.append('action', 'mhub_google_meeting_list');
                ...
                if (response.data.success) {
                    if (Array.isArray(response.data.data)) {
                        ...
                        const googleMeetings = response.data.data.map(meeting => ({
    ...
    return (
        <Fragment>
            ...
            <div>
                <h3>{__('Show Direct Google Meeting', 'meetinghub')}</h3>

                { selectedMeeting && linkOnly ? (
                    <a className="mhub_join_btn" href={selectedMeeting.hangoutLink} target="_blank" rel="noopener noreferrer">
                       {__('Join meeting', 'meetinghub')}
                    </a>
                ) : (
                     selectedMeeting && (
                        <table className="mhub-table">
                           <tbody>
                                ...
                                <tr>
                                    <td>{__('Topic', 'meetinghub')}</td>
                                    <td>{selectedMeeting.summary}</td>
                                </tr>
                                <tr>
                                    <td>{__('Start Time', 'meetinghub')}</td>
                                    <td>{formatDateTime(selectedMeeting.start.dateTime)}</td>
                                </tr>
                                <tr>
                                    <td>{__('Timezone', 'meetinghub')}</td>
                                    <td>{selectedMeeting.start.timeZone}</td>
                                </tr>
                                <tr>
                                    <td>{__('Join Link', 'meetinghub')}</td>
                                    <td><a href={selectedMeeting.hangoutLink} target="_blank" rel="noopener noreferrer">{__('Join', 'meetinghub')}</a></td>
                                </tr>
                            </tbody>
                        </table>
                    )
...
// Register GoogleMeet block
registerBlockType('meetinghub/googlemeet', {
