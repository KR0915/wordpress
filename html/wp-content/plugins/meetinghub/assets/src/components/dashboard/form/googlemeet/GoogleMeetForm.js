...
const GoogleMeetForm = ({ selectedPlatform }) => {
	const [errorMessage, setErrorMessage] = useState('');
	const [isSaving, setIsSaving] = useState(false);
	const navigate = useNavigate();
	const hiddenSubmitRef = useRef(null);
	var mhub_google_meet_settings = mhubMeetingsData.mhub_google_meet_settings;
	const defaultTimezone = mhub_google_meet_settings ? mhub_google_meet_settings.meeting_timezone : mhubMeetingsData.mhub_timezone;
	...
	const [formData, setFormData] = useState({
		...
		duration_hours: 1,
		duration_minutes: 40,
		reminder_time: mhub_google_meet_settings ? mhub_google_meet_settings.reminder_time : 1,
		event_status: mhub_google_meet_settings ? mhub_google_meet_settings.event_status : 'tentative',
		send_updates: mhub_google_meet_settings ? mhub_google_meet_settings.send_updates : 'none',
		transparency: mhub_google_meet_settings ? mhub_google_meet_settings.transparency : 'transparent',
		event_visibility: mhub_google_meet_settings ? mhub_google_meet_settings.event_visibility : 'default',
		hide_sidebar: mhub_google_meet_settings ? mhub_google_meet_settings.hide_sidebar : false,
		hide_header_footer: mhub_google_meet_settings ? mhub_google_meet_settings.hide_header_footer : false,
		enable_should_register: mhub_google_meet_settings ? mhub_google_meet_settings.enable_should_register : false,
		enable_recurring_meeting: mhub_google_meet_settings ? mhub_google_meet_settings.enable_recurring_meeting : false,
		recurrence_option: 1,
		end_type: 'date',
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (!response) {
				toast.error(langString('failed_create_meeting'));
				setErrorMessage(langString('check_meet_settings') );
			}
...
// End Type
	...
	useEffect(() => {
		if ( 'connected' != mhubMeetingsData.google_account && mhubMeetingsData.is_admin) {
			setErrorMessage(
				langString('google_meet_api_setup_notice')  +
				`<a href="admin.php?page=meetinghub-settings#/google_meet">Configure Google Meet</a>.`
			);
		}
	}, [mhubMeetingsData.google_account, mhubMeetingsData.is_admin]);
